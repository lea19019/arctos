#!/usr/bin/env bash
#SBATCH --job-name=arctos-an-bloom
...
#SBATCH --output=logs/analysis_bloom_%j.out
#SBATCH --error=logs/analysis_bloom_%j.err
set -euo pipefail
...
.venv/bin/python -u experiments/q1-language-emergence/pivot_runner.py \
    ...
    --output results/bloom-7b1/pivot --n-examples 100 --pairs en-zh en-arz cs-de
echo "--- attention viz ---"
.venv/bin/python -u experiments/q2-attention-heads/attention_viz.py \
    --config experiments/q1-language-emergence/configs/bloom.yaml \
    --q1-results results/bloom-7b1/q1 --output results/bloom-7b1/q2/attention --top-k 6 || echo "attn-viz failed (non-fatal)"
echo "===== done $(date) ====="
