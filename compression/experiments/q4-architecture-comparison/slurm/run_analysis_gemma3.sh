#!/usr/bin/env bash
#SBATCH --job-name=arctos-an-gemma3
...
#SBATCH --output=logs/analysis_gemma3_%j.out
#SBATCH --error=logs/analysis_gemma3_%j.err
set -euo pipefail
...
.venv/bin/python -u experiments/q1-language-emergence/pivot_runner.py \
    ...
    --output results/gemma-3-12b-it/pivot --n-examples 100 --pairs en-zh en-arz cs-de
echo "--- attention viz ---"
.venv/bin/python -u experiments/q2-attention-heads/attention_viz.py \
    --config experiments/q1-language-emergence/configs/gemma3.yaml \
    --q1-results results/gemma-3-12b-it/q1 --output results/gemma-3-12b-it/q2/attention --top-k 6 || echo "attn-viz failed (non-fatal)"
echo "===== done $(date) ====="
