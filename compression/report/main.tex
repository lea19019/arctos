...
stream} \citep{elhage2021framework}, the additive running sum that flows down a
...
\paragraph{Data.} FLORES+ dev \citep{nllb2022}, three language pairs chosen for contrast:
...
\midrule
Aya Expanse 8B \citep{dang2024aya} & 2024 & Cohere, RoPE, RMSNorm & general multilingual LM \\
TowerBase 7B \citep{alves2024tower} & 2024 & Llama-2 & + bilingual CPT (no SFT) \\
TowerInstruct 7B \citep{alves2024tower} & 2024 & Llama-2 & + MT-task SFT \\
Tower-Plus 9B \citep{rei2025towerplus} & 2025 & \textbf{Gemma 2}~\citep{gemmateam2024gemma2} & CPT + MT-SFT \\
BLOOM 7B1 \citep{lescao2022bloom} & 2022 & \textbf{ALiBi, LayerNorm} & multilingual pretraining \\
EuroLLM 9B \citep{martins2024eurollm} & 2024 & Llama-3 & European MT specialist \\
Llama-3.1 8B \citep{dubey2024llama3} & 2024 & Llama-3, GQA & general LM \\
Gemma-3 12B \citep{gemmateam2025gemma3} & 2025 & Gemma 3 & baseline (Google QAT) \\
\midrule
NLLB-200 3.3B \citep{nllb2022} & 2022 & \textbf{encoder--decoder} & MT-purpose-built \\
\bottomrule
\end{tabular}
...
$<\!10^{-4}$.) A learned refinement, the tuned lens \citep{belrose2023tunedlens},
trades calibration for added trainable parameters; we use the parameter-free
logit lens throughout.

\paragraph{Probing.} Train a linear classifier per layer \citep{alain2017probes}
to predict a feature
(source/target language) from the residual; report \emph{selectivity} =
accuracy $-$ control-task accuracy \citep{hewitt2019control}, which separates
``the feature is represented'' from ``the probe memorized.'' Reveals
\emph{where} language identity becomes linearly decodable.

\paragraph{Information Flow Routes (IFR).} Following \citet{ferrando2024ifr},
...
unembed \citep{elhage2021framework}: $\text{DLA}(c)=(c\odot s)\cdot W_U[:,t]$. Signed: positive = pushes
...
activation patching \citep{kramar2024atp}: $\text{effect}(c)\approx \partial L/\partial a_c \cdot
...
directly simulating quantization error \citep{frantar2023gptq,lin2024awq}.
Reveals where numerical \emph{precision} matters.
...
\citep[cf.][]{wendler2024llamas,dumas2024tongue}.
...
DLA isolates recurring heads. In Aya, head L30.H8 is a strong cross-pair
\emph{predictor} (DLA $+0.72$ on cs$\rightarrow$de, $+0.44$ on
en$\rightarrow$arz); L29.H16 and L26.H5 act as \emph{suppressors} (clearly
negative on cs$\rightarrow$de, $-0.22$ and $-0.16$, but near zero on
en$\rightarrow$arz, $-0.02$/$-0.06$ --- so the suppression is strong on the
pair the model translates best and weak on the hard low-resource pair). The signed DLA depth curve
...
Per-layer & whole layer's weights & \textbf{chrF++ drop}~\citep{popovic2017chrf} & $-0.065$ (IFR), $-0.046$ (DLA) \\
\bottomrule
\end{tabular}
