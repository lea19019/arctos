...
Phase one is an interpretability-led investigation that precedes any compression method. Thesis spine, model choices, language pairs, and methodological discipline are in [`docs/archive/project_summary.md`](../archive/project_summary.md). Prior work is `docs/pruning_project.pdf`. Phase-two candidate directions are seeded in [`docs/archive/phase2_hypotheses.md`](../archive/phase2_hypotheses.md) and stay frozen until Q5 closes.
...
**Expected artifacts.** Per-model layer charts (probe accuracy + selectivity, target-token mass under logit lens) under `compression/experiments/q1-language-emergence/`; synthesis in `compression/docs/q1_language_emergence.md`.
...
**Expected artifacts.** Per-model ranked head list with patching effect size; informal characterization (source-attender, target-predictor, language-router, ...) for top ~10 heads; `compression/docs/q2.md`.
...
**Expected artifacts.** Per-model depth profile of layer-level MLP MT-importance (one column per method, so disagreement is visible); cross-model comparison in `compression/docs/q3.md`.
...
**Expected artifacts.** `compression/docs/q4_architecture_comparison.md` — paper-style writeup pitched at someone unfamiliar with the project.
...
**Expected artifacts.** Per-component (importance × sensitivity) map for at least Aya; `docs/archive/phase2_hypotheses.md` updated from a seed doc into a hypothesis-shaped writeup the phase-two design draws from.

**Satisfied when.** A per-component map exists, and `docs/archive/phase2_hypotheses.md` has been updated with phase-one evidence supporting or killing each candidate.
...
Every method, loader, data generator, and metric wrapper that lands in `compression/src/` ships with tests in `compression/tests/` mirroring the source layout. Tests are first-class — pytest is a core dependency, not optional.
...
The `cpu` / `gpu` / `slow` markers and conventions are configured in `pyproject.toml` and `compression/tests/conftest.py`.
...
| COMET wmt22-comet-da misjudges en→ar-arz translations. | All | Report COMET alongside chrF++ and a small human-judgment spot check; document in `compression/src/eval/`. |

## Phase 2

Empty by design. To be designed from Q5 findings against the candidates seeded in `docs/archive/phase2_hypotheses.md`. New candidates may emerge from phase-one findings.
