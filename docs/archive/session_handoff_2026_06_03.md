...
Persist each to `compression/docs/deep_research_raw/<taskid>.txt` when done, then
...
- **Novel direction (the thesis):** `compression/docs/phase2_novel_direction.md` —
...
1. Read + persist the 3 running deep-research outputs; update
   `compression/docs/phase2_method_primer.md` / `phase2_novel_direction.md` with verdicts.
2. GPU-canary the `pipeline` stage; if `crush_middle >> crush_ends`, the novel
   method holds → run across models, write `compression/docs/q6-pipeline.md`.
