...
| Track | State | Where |
|---|---|---|
| Interpretability of MT (phase one) | Complete, written up as a paper | `compression/experiments/q1…q5`, `compression/docs/` |
...
results and no writeup and never did. The old `docs/findings/README.md` listed
`q2.md` and `q3.md`; neither has ever existed in git history. *(Fixed
2026-08-06: the replacement `compression/docs/README.md` states this explicitly.
The experiment folders' own READMEs still list them as expected artifacts.)*
...
behave oppositely. `docs/archive/advisor_brief.md` conflates them; it is the drifted doc.
...
Two survey documents (`speech-translation/docs/nllb_xtts_compression_survey.md`,
`nllb_xtts_interp_map.md`) specify ~11 experiments. **Essentially none were run**
...
- `compression/docs/q1_language_emergence.md` reports head L26.H5 at **−0.18 on en→arz**. The data says
  **−0.058**; −0.177 is its *en→zh* value. The paper was corrected for this
  (commit `ff2fe04`); the findings doc was not. **`main.tex` and `q1_language_emergence.md` now
  contradict each other**, with the paper matching the data.
- `phase2_results.md` gives tower-base W3 COMET as .32; disk says 0.315.

**Coverage that is overstated**

- **Gemma and BLOOM have no GPTQ data at all.** `phase2_results.md` blames
...
- **Gemma-3 is absent from both findings docs** despite being the paper's most
  extreme data point.
- **NLLB is simultaneously "deferred" (`q1_language_emergence.md`) and a headline generalization
  result** (`q4_architecture_comparison.md`, the paper).
...
- **`archive/phase2_hypotheses.md` was never updated**, though Q5's satisfied-when
  ...
  the one that won.
- **`phase2_novel_direction.md` still reads as a live proposal** with no results
