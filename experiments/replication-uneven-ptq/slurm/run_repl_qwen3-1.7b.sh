#!/usr/bin/env bash
#SBATCH --job-name=repl-qwen3-1.7b
...
#SBATCH --output=logs/repl_qwen3-1.7b_%j.out
#SBATCH --error=logs/repl_qwen3-1.7b_%j.err
set -euo pipefail
...
.venv/bin/python -u experiments/replication-uneven-ptq/experiment.py \
    --config experiments/replication-uneven-ptq/configs/qwen3-1.7b.yaml \
    --output results/replication-uneven-ptq/qwen3-1.7b
echo "===== done $(date) ====="
