...
2. **From-scratch training + log-spaced checkpointing, 130GB × 15 runs** — W3 budgets *one week
   ...
   found mid-run, z-loss needed because `lm_head` exploded. Plus the documented
   `run_with_cache` batch-size-1 bug, which must be fixed before 60 × 15 checkpoint passes.
