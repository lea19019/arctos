...
def forward_pass(model, enc, layers):
    ...
    handles = [down_proj(l).register_forward_hook(make_hook(i))
...
def main():
    ...
    for rnd in range(args.max_rounds):
        ...
        for sa in sas:
            X, Y = store[sa["layer"]]
            cands, why = decompose(sa, X, Y, down_proj(layers[sa["layer"]]).weight,
                                   args)
        ...
        for sa, c in fresh:
            ...
            with torch.no_grad():
                down_proj(layers[coord[0]]).weight[coord[1], coord[2]] = 0.0
        if over:
            break

    with torch.no_grad():
        for f in found:
            down_proj(layers[f["layer"]]).weight[f["j"], f["k"]] = f["value"]
