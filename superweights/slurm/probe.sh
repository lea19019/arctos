...
.venv/bin/python src/detect_sw.py --model "$MODEL" --out "$OUT/${SLUG}_found.json"
...
.venv/bin/python - "$OUT/${SLUG}_ablation.json" <<'PY'
import json, sys
...
print(f"probe info: dtype={d['dtype']} device={d['device']} git={d['git_sha']}")
