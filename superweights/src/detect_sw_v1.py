"""Model-agnostic super-weight detector -- FROZEN v1, kept for comparison.

This is the first-generation detector, restored verbatim from commit 354ba22
so its behaviour stays runnable and re-measurable. The live detector is
`detect_sw.py` (v2), which fixes the four miss causes this version's own
instrumented run exposed. Do not edit this file to "improve" it -- that is
what v2 is for; the point of keeping it is that v1's numbers are
reproducible.

Detector lineage in this track:
  olmo_sw.py       v0  scratch, OLMo-1B only, written from the paper alone
  detect_sw_v1.py  v1  model-agnostic; 11 of 21 Table 2 coordinates
  detect_sw.py     v2  top-j + contribution decomposition + peel-all

Its default output path is results/v1/ so it cannot overwrite v2's results.
...
def main():
    ...
    out = Path(args.out) if args.out else (
        Path("results/v1") / (args.model.replace("/", "_") + "_found.json"))
